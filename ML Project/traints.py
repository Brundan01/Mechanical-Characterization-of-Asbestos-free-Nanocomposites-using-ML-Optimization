...
# Load data from CSV
data = pd.read_csv('findatasetts.csv')

# Preprocessing
X = data.drop('Tamerind', axis=1)
y = data['Tamerind']
...
joblib.dump(rf_classifier, 'rfts.pkl')
joblib.dump(logistic_reg, 'logts.pkl')
joblib.dump(linear_reg, 'lrts.pkl')

with open('rfts.pkl', 'wb') as f:
    pickle.dump(rf_y_pred, f)

with open('logts.pkl', 'wb') as f:
    pickle.dump(logistic_y_pred, f)

with open('lrts.pkl', 'wb') as f:
