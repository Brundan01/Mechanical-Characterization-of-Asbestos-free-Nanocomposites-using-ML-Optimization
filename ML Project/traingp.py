...
# Load data from CSV
data = pd.read_csv('findatasetgp.csv')

# Preprocessing
X = data.drop('Graphene', axis=1)
y = data['Graphene']
...
joblib.dump(rf_classifier, 'rfgp.pkl')
joblib.dump(logistic_reg, 'loggp.pkl')
joblib.dump(linear_reg, 'lrgp.pkl')

with open('rfgp.pkl', 'wb') as f:
    pickle.dump(rf_y_pred, f)

with open('loggp.pkl', 'wb') as f:
    pickle.dump(logistic_y_pred, f)

with open('lrgp.pkl', 'wb') as f:
