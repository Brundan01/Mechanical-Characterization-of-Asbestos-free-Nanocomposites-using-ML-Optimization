...
# Load data from CSV
data = pd.read_csv('findatasetr.csv')

# Preprocessing
X = data.drop('Ridge', axis=1)
y = data['Ridge']
...
joblib.dump(rf_classifier, 'rfrgp.pkl')
joblib.dump(logistic_reg, 'logrgp.pkl')
joblib.dump(linear_reg, 'lrrgp.pkl')

with open('rfrgp.pkl', 'wb') as f:
    pickle.dump(rf_y_pred, f)

with open('logrgp.pkl', 'wb') as f:
    pickle.dump(logistic_y_pred, f)

with open('lrrgp.pkl', 'wb') as f:
